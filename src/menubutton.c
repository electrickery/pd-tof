...
static int menubutton_set_float(t_menubutton* x, t_floatarg item)
{
    ...
    if ((i < x->x_num_options) && (i >= 0))
    {
        ...
        if (menubutton_w_is_visible(x))
            menubutton_w_text(x,x->x_options[x->current_selection]);
        return 1;
    }
    else
    {
...
static void *menubutton_new(t_symbol *s, int argc, t_atom *argv)
{
    ...
    {
        if ((argv+conf_argc)->a_type == A_SYMBOL && 
            atom_getsymbol(argv+conf_argc) == Comma)
        {
            break;
        }
    }
    ...
    switch(conf_argc){ 
        case 10: if ((argv + 9)->a_type == A_FLOAT)  
            x->halign = atom_getfloat(argv + 9);
        case 9:  if ((argv + 8)->a_type == A_SYMBOL)
            x->co_color = atom_getsymbol(argv + 8);
        case 8:  if ((argv + 7)->a_type == A_SYMBOL)
            x->hi_color = atom_getsymbol(argv + 7);
        case 7:  if ((argv + 6)->a_type == A_SYMBOL)
            x->fg_color = atom_getsymbol(argv + 6);
        case 6:  if ((argv + 5)->a_type == A_SYMBOL)
            x->bg_color = atom_getsymbol(argv + 5);
        case 5:  if ((argv + 4)->a_type == A_FLOAT)
            x->saveitems = atom_getfloat(argv + 4);
        ...
        case 2:  if ((argv + 1)->a_type == A_FLOAT)
            x->x_height = atom_getfloat(argv + 1);  
        case 1:  if ((argv)->a_type == A_FLOAT) 
            x->x_width = atom_getfloat(argv);  
            break;
    }
    ...
    {
        if (((item_argv+i)->a_type == A_SYMBOL && 
            atom_getsymbol(item_argv + i) == Comma) || 
            i == item_argc ) 
        {
            if ( buffer_size ) {
...
void menubutton_zoom(t_menubutton *x, t_floatarg zoom)
{
...
void menubutton_setup(void) {
    ...
    menubutton_class = class_new(gensym("menubutton"), 
    ...
    class_addmethod(menubutton_class, (t_method)menubutton_zoom,
